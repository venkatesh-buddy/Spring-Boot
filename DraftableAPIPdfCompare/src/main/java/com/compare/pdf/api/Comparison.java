...
public final class Comparison {
    ...
    public static final class Side {
        ...
        /**
         * Creates a representation of a file passed as the left or right side of a comparison.
    ...
    @Nonnull private final Side left;

    /** Metadata for the right file. */
    @Nonnull private final Side right;
    ...
    @Nullable private final Instant expiryTime;

    /** Whether the comparison has been processed. */
    private final boolean ready;

    /** If the comparison is ready, gives the time at which processing finished. */
    @Nullable private final Instant readyTime;

    /** Whether the comparison failed. Is null if the comparison hasn't been processed. */
    @Nullable private final Boolean failed;
     ...
     */
    public Comparison(@Nonnull final String identifier,
                      @Nonnull final Side left,
                      @Nonnull final Side right,
                      boolean isPublic,
                      @Nonnull final Instant creationTime,
                      @Nullable final Instant expiryTime,
                      final boolean ready,
                      @Nullable Instant readyTime,
                      @Nullable final Boolean failed,
    ...
    //region Public getter methods: getIdentifier(), getLeft(), getRight(), getIsPublic(), getCreationTime(), getExpiryTime(), getReady(), getFailed(), getErrorMessage()
     ...
     */
    @Nullable
    public final Instant getReadyTime() { return readyTime; }

    /**
     * Gets whether the comparison failed. Is null if the comparison hasn't been processed (i.e. isn't ready).
     * @return true if the comparison failed, false if the comparison is ready and succeeded, and null if the comparison isn't ready.
     */
    @Nullable
     ...
     * @return A message describing the error if the comparison has failed. Null if the comparison isn't ready or succeeded.
     */
    @Nullable
    ...
    //endregion Public getter methods: getIdentifier(), getLeft(), getRight(), getIsPublic(), getCreationTime(), getExpiryTime(), getReady(), getFailed(), getErrorMessage()
    ...
    public final String toString() {
        return String.format(
            ...
            expiryTime == null ? "null" : expiryTime.toString(),
            ready ? "true" : "false",
